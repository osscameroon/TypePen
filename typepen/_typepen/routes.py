...
def home():
    ...
    for adf in all_dir_files:
        if adf.endswith(".typen"):
            file_details[adf] = time.strftime(
                "%Y-%m-%d %H:%M:%S",
                time.localtime(os.path.getmtime(os.path.join(PATH_TO_ALL_FILES, adf))),
            )

    return render_template(
        "main.html", file_details=file_details, dpath=PATH_TO_ALL_FILES
    )
